...
const tarSource = (events, path, options) => {
  ...
  untar.on('entry', (header, fileStream, next) => {
    ...
    const skip = () => fileStream.resume()
    const stream = () => fileStream

    if (header.type === 'file')
      setImmediate(() => events.emit(header.type, {path, skip, stream}))
    else
      skip()
  })
...
const dirSource = (events, path, options) => {
  ...
  walker.on('file', (root, fileStats, next) => {
    ...
    if (fileStats.type === 'file') {
      ...
      setImmediate(() => events.emit('file', {path, skip, stream}))
    } else {
      console.warn(`Ignoring type '${yellow(fileStats.type)}' for path ${path}`)
      setImmediate(next)
    }
  })
...
const bundle = (path, options) => {
  ...
  fs.stat(path, (err, stats) => {
    if (err) {
      const events = new EventEmitter()
      //console.error(red(`Error pulling stats on file:`), err)
