...
function runPs (dir, options) {
  ...
  const source = fs.createReadStream(psFile)

  psJson(source, options)
}

function psJson(source, {filter, extractor, handler}) {
  let nodeCount = 0
  ...
  .node('!.*', node => {
    ...
    if (filter && !filter(node)) {
      // Only forward nodes which pass the filter (if present).
      return oboe.drop;
    } else {
